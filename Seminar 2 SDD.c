...
struct Film initializare(int _id, int _durata, const char* _nume, float _buget, char _varstaMinima) {
	struct Film s;
	s.id = _id;
	s.durata = _durata;
...
int main() {
	struct Film f1 = initializare(1, 120, "Dune", 20.3, 14);
	afisare(f1);
	...
	filme[0] = f1;
	*(filme + 1) = initializare(2, 200, "Spiderman", 34.2, 14);
	filme[2] = initializare(3, 140, "IronMan", 40.5, 16);
